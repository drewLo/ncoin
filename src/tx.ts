...
const newUnspentTxOuts: UnspentTxOut[] = newTransactions
    .map(t => {
        return t.txOuts.map(
            (txOut, index) =>
                new UnspentTxOut(t.id, index, txOut.addresss, txOut.amount)
        );
    })
...
const isValidTxOutStructure = (txOut: TxOut): boolean => {
    if (txOut == null) {
        ...
        console.log("invalid amount typ in txOut");
        return false;
    } else {
        return true;
    }
...
const signTxIn = (
    ...
    const dataToSign = transaction.txIns;
...
const validateCoinbaseTx = (
    ...
    if (transaction.txIns.length !== 1) {
        console.log("one txIn must be specified in th coinbas transaction");
        return;
    }
